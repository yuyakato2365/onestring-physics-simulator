...
def test_ceps_obj_rejects_translated_internal_cut_instead_of_stitching_it(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(RuntimeError, match="internal cut seams"):
        _parse_ceps_obj(path)


def test_ceps_obj_without_cut_keeps_original_surface_connectivity(
    tmp_path: Path,
) -> None:
    ...
    assert metrics["ceps_ordinary_uv_chart_already_single_valued"] is True
    assert metrics["ceps_continuous_chart_reconstructed"] is False
    assert metrics["ceps_internal_cut_seam_edge_count"] == 0
    assert metrics["ceps_internal_cut_seams_stitched"] is False
    assert metrics["ceps_internal_cut_seams_supported"] is False
...
def test_ceps_obj_rejects_non_isometric_cut_without_hiding_it(
    tmp_path: Path,
) -> None:
