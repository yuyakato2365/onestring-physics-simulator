...
CANONICAL_PATCH_VERSION = "4.7-fixedvert-hard-lock"
...
def _patch_optimizer_hard_grid_locks(root: Path) -> bool:
    """Make every OptCuts fixed vertex a hard no-motion invariant.

    Grid seam vertices are inserted into fixedVert by the core patch.  Using
    fixedVert here is stronger and safer than depending only on the auxiliary
    oneStringGridLockedVert set, because fixedVert is the native OptCuts state
    that is propagated into the solver and scaffold machinery.
    """
    ...
    text = _replace_once(
        ...
        """        else {\n            linSysSolver->solve(minusG, searchDir);\n        }\n        if(!mute) { timer_step.stop(); }\n\n        // ONESTRING_GRID_NATIVE_V4_ZERO_LOCKED_SEARCH_DIR\n        // fixedVert is the authoritative OptCuts solver constraint set.\n        for(const int vI : result.fixedVert) {\n            if(vI >= 0 && vI < result.V.rows() && vI * 2 + 1 < searchDir.size()) {\n                searchDir[vI * 2] = 0.0;\n                searchDir[vI * 2 + 1] = 0.0;\n            }\n        }\n        \n        fractureInitiated = false;\n""",
        "Optimizer hard-zero fixed/Grid lock search direction",
    )
    ...
    text = _replace_once(
        ...
        "Optimizer hard-skip fixed/Grid lock stepForward",
    )

    path.write_text(text, encoding="utf-8")
    print(f"Applied hard OptCuts fixed/Grid optimizer locks: {path}")
    return True
