    ...
    bool prettyPrint)
{
    ...
    model.asset.generator = "ABD";

    Scene& scene = model.scenes.emplace_back();
    scene.name = "ABDSimulation";
    ...
    for (int i = 0; i < num_bodies; i++) {
        ...
        Eigen::Quaternion<double> q = construct_quaternion_from_transform(poses[0][i].transform);
    ...
    for (int i = 0; i < num_bodies; i++) {
        ...
        for (int j = 0; j < num_steps; j++) {
            Eigen::Quaternion<double> quat = construct_quaternion_from_transform(poses[j][i].transform);
