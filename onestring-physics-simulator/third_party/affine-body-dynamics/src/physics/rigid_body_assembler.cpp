...
void RigidBodyAssembler::init(const std::vector<RigidBody>& rigid_bodies)
{
    ...
    for (int i = 0; i < m_rbs.size(); i++) {
        std::string temp_name = m_rbs[i].name;

        // Append name with id if it already exists (increasing id until unique)
        int count = 0;
        while (body_name_id_map.find(temp_name) != body_name_id_map.end()) {
            count++;
            temp_name = temp_name + std::to_string(count);
        }
        if (count > 0) {
            spdlog::warn(
                "Duplicate body name: " + m_rbs[i].name + ". Changed it to "
                + temp_name);
        }
        m_rbs[i].name = temp_name;
        body_name_id_map[temp_name] = long(i);
    }
    ...
    for (size_t n_i = 0; n_i < is_rb_dof_fixed.size(); n_i++)
        is_rb_dof_fixed(n_i) = false;
    ...
    average_mass = m_rb_mass_matrix.diagonal().mean();

    // derivative of world vertices wrt pose (is constant)
    init_world_vertices_jacobian();
}
...
void RigidBodyAssembler::init_world_vertices_jacobian()
{
    // Jac: Rᵐ ↦ Rⁿ (vertices flattened rowwise)
    const int n = num_vertices() * dim();
    const int m = PoseD::dim_to_ndof(dim());

    m_world_vertices_jac.resize(n, m);
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<size_t>& range) {
            for (size_t rb_i = range.begin(); rb_i != range.end(); ++rb_i) {
                ...
                rb.world_vertices_jacobian(rb_v0_i, m_world_vertices_jac);
            }
        });
}

Eigen::MatrixXd RigidBodyAssembler::world_vertices_diff(
    const PosesD& poses, Eigen::MatrixXd& jac, bool compute_jac) const
{
    assert(num_bodies() == poses.size());
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<size_t>& range) {
            for (size_t rb_i = range.begin(); rb_i != range.end(); ++rb_i) {
                ...
                rb.world_vertices(poses[rb_i], rb_v0_i, V);
            }
        });

    jac = m_world_vertices_jac;
...
std::vector<std::pair<int, int>> RigidBodyAssembler::close_bodies_iaabb(
    const PosesD& poses_t0,
    const PosesD& poses_t1,
    const double inflation_radius) const
{
    ...
    for (int i = 0; i < num_bodies(); i++) {
        VectorMax3d min, max;
        m_rbs[i].compute_spatial_rotation_bbox(
            poses_t0[i], poses_t1[i], min, max);
        min.array() -= inflation_radius;
        max.array() += inflation_radius;
        Eigen::Vector3d min3D = Eigen::Vector3d::Zero(),
                        max3D = Eigen::Vector3d::Zero();
...
// @javidf: Not sure if this method is needed. In general, we should try to
// combine methods that use a single pose with those that use two poses for
// collision detection.
std::vector<std::pair<int, int>> RigidBodyAssembler::close_bodies_iaabb(
    const PosesD& poses, const double inflation_radius) const
{
    std::vector<std::array<Eigen::Vector3d, 2>> body_bounding_boxes;
    body_bounding_boxes.reserve(num_bodies());
    ...
    for (int i = 0; i < num_bodies(); i++) {
        VectorMax3d min, max;
        min = poses[i].position.array() - m_rbs[i].r_max;
        max = poses[i].position.array() + m_rbs[i].r_max;
        // m_rbs[i].compute_spatial_rotation_bbox(poses_t0[i], poses_t1[i], min,
        // max);
        min.array() -= inflation_radius;
        max.array() += inflation_radius;
        Eigen::Vector3d min3D = Eigen::Vector3d::Zero(),
