...
{
    PROFILE_POINT("IPCSolver::init_solve");
    PROFILE_START();
    ...
    double org_orth_stiff = barrier_problem_ptr()->orthogonal_stiffness();
    // barrier_problem_ptr()->orthogonal_stiffness(0.0);
    barrier_problem_ptr()->compute_energy_term(x0, grad_E);
    // barrier_problem_ptr()->orthogonal_stiffness(org_orth_stiff);
...
void IPCSolver::post_step_update()
{
    ...
    //  barrier_problem_ptr()->barrier_stiffness(1e8);
