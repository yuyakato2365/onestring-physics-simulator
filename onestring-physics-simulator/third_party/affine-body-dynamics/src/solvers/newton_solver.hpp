...
public:
    ...
    /**
     * @brief Perform Newton's Method to minimize the objective, \f$f(x)\f$,
     * of the problem while satisfying constraints that are linear wrt x.
     *
     * @param[in] problem  The optimization problem to minimize
     *                     unconstrained.
     *
     ...
     * minimum, and if the optimization was successful.
     */
    OptimizationResults solve_w_linear_constraints(const Eigen::VectorXd& x0);
...
protected:
    ...
    Eigen::VectorXd grad_constrained; ///< Gradient with constrained directions subtracted
