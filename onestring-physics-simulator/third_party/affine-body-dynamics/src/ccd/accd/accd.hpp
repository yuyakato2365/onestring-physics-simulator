    ...
    T eta, T& toc)
{
    ...
    for (int i = 0; i < rootAmt; ++i) {
        if (roots[i] > 0 && roots[i] <= 1) {
            ...
            //     return true;
            // }
            if(ipc::point_edge_distance_type(Eigen::Matrix<T, 2, 1>(x0 + roots[i] * d0), 
            Eigen::Matrix<T, 2, 1>(x1 + roots[i] * d1),
            Eigen::Matrix<T, 2, 1>(x2 + roots[i] * d2) ) == ipc::PointEdgeDistanceType::P_E) {
                toc = roots[i] * (1 - eta); //TODO: distance eta
                return true;
            }
        }
...
{
    Eigen::Matrix<T, 3, 1> mov = (dt0 + dt1 + dt2 + dp) / 4.0;
    dt0 -= mov;
    dt1 -= mov;
    dt2 -= mov;
    dp -= mov;
    ...
    T dist2_cur = ipc::point_triangle_distance(p, t0, t1, t2);
    ...
    while (true) {
        ...
        // Point_Triangle_Distance_Unclassified(p, t0, t1, t2, dist2_cur);
        dist2_cur = ipc::point_triangle_distance(p, t0, t1, t2);
    ...
    T eta, T thickness, T& toc)
{
    ...
    Eigen::Matrix<T, 3, 1> mov = (dea0 + dea1 + deb0 + deb1) / 4.0;
    dea0 -= mov;
    dea1 -= mov;
    deb0 -= mov;
    deb1 -= mov;
    ...
    T dist2_cur = ipc::edge_edge_distance(ea0, ea1, eb0, eb1);
    // double init_dist = std::sqrt(dist2_cur);
    // std::cout << "ACCD : \n ea0: \n" << ea0 << "\n ea1: \n" << ea1 << "\n eb0: \n" << eb0 << "\n eb1: \n" << eb1 << "\n distance: d" << dist2_cur << std::endl;
    // std::cout << "dist2_cur : " << dist2_cur << std::endl;
    ...
    while (true) {
        ...
        if (toc && (dFunc / (dist_cur + thickness) < gap)) {
            // std::cout << "toc: " << toc << " and ddFunc : " << dFunc << std::endl;
            break;
        }
...
bool Point_Edge_CCD(
    ...
    T eta, T thickness, T& toc)
{
    Eigen::Matrix<T, 3, 1> mov = (dp + de0 + de1) / 3.0;
    de0 -= mov;
    de1 -= mov;
    dp -= mov;
    ...
    T dist2_cur = ipc::point_edge_distance(p, e0, e1);
    T dist_cur = std::sqrt(dist2_cur);
    T gap = eta * (dist2_cur - thickness * thickness) / (dist_cur + thickness);
    T toc_prev = toc;
    toc = 0;
    while (true) {
        ...
        // Point_Edge_Distance_Unclassified(p, e0, e1, dist2_cur);
        dist2_cur = ipc::point_edge_distance(p, e0, e1);
        dist_cur = std::sqrt(dist2_cur);
        if (toc && (dist2_cur - thickness * thickness) / (dist_cur + thickness) < gap) {
            break;
        }
...
bool Point_Point_CCD(
    ...
    T eta, T thickness, T& toc)
{
    Eigen::Matrix<T, 3, 1> mov = (dp0 + dp1) / 2.0;
    dp1 -= mov;
    dp0 -= mov;
    ...
    T dist2_cur = ipc::point_point_distance(p0, p1);
    T dist_cur = std::sqrt(dist2_cur);
    T gap = eta * (dist2_cur - thickness * thickness) / (dist_cur + thickness);
    T toc_prev = toc;
    toc = 0;
    while (true) {
        ...
        // Point_Point_Distance(p0, p1, dist2_cur);
        dist2_cur = ipc::point_point_distance(p0, p1);
        dist_cur = std::sqrt(dist2_cur);
        if (toc && (dist2_cur - thickness * thickness) / (dist_cur + thickness) < gap) {
            break;
        }
