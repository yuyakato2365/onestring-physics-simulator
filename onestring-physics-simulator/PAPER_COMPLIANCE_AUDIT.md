...
| S -> Omega parameterization | Paper uses a conformal/BFF-style surface parameterization. | Default is `bff`: a local boundary-first solve that builds a boundary candidate from 3D boundary edge lengths and signed turning angles, coerces that `Omega` boundary onto a rectangle by 3D boundary arclength, then solves the interior with cotangent harmonic extension. `lscm_paper_like` is retained only as an explicit comparison mode. `pca_debug`, `shape_preserving_experimental`, and `rectangular_debug` require `allow_experimental_pipeline=True`. | `_build_surface_parameterization`, `_bff_boundary_first_uv`, `_bff_boundary_polygon`, `_rectangularize_boundary_by_arclength` in `onestring_pipeline.py` | Local BFF-style implementation with rectangular boundary correction | This is no longer PCA/height-field projection, but it intentionally applies rectangular boundary correction and is not a libigl reference BFF binding; numerical equivalence to the paper must be checked with `bff_*` metrics. | `parameterization_method`, `parameterization_exactness_label`, `omega_boundary_constraint_model`, `bff_variant`, `bff_boundary_*` | Compare against a reference BFF implementation if exact reproducibility is required. |
| Omega boundary | Boundary should come from the conformal solve unless rectangular fabrication/debug constraints are desired. | Default BFF now forces the Omega boundary onto a rectangle. `shape_preserving_experimental` preserves projected outline only in explicit experimental mode; `rectangular_debug` is explicitly debug. | `_rectangularize_boundary_by_arclength`, `_shape_preserving_projected_uv` | Rectangular correction / Experimental / Debug | Rectangular coercion stabilizes Omega/M2D framing but can distort conformal scale near the boundary. Metrics expose the correction and edge-length error. | `omega_boundary_mode`, `omega_boundary_forced_rectangle`, `omega_boundary_constraint_model`, `bff_boundary_rectangular_correction_applied`, `bff_boundary_rect_max_relative_length_error`, `boundary_self_intersection_count` | Add reference BFF comparison and flip-prevention diagnostics. |
