...
| S -> Omega parameterization | Paper uses a conformal/BFF-style surface parameterization. | Default is `lscm_paper_like`: a free-boundary LSCM solve with two pinned boundary vertices. `pca_debug`, `shape_preserving_experimental`, and `rectangular_debug` require `allow_experimental_pipeline=True`. | `_build_surface_parameterization`, `_lscm_free_boundary_uv` in `onestring_pipeline.py` | Conformal approximation / BFF unimplemented | LSCM is conformal, but if the paper specifically requires BFF this is still not a full paper implementation. | `parameterization_method`, `parameterization_exactness_label`, `omega_boundary_constraint_model`, `lscm_lsqr_residual_norm` | Confirm paper-required parameterization and replace LSCM with BFF if needed. |
| Omega boundary | Boundary should come from the conformal solve, not arbitrary square forcing. | Default LSCM uses a free boundary and pins only two vertices. `shape_preserving_experimental` preserves projected outline only in explicit experimental mode; `rectangular_debug` is explicitly debug. | `_lscm_free_boundary_uv`, `_shape_preserving_projected_uv` | Conformal approximation / Experimental / Debug | Two-pin LSCM can still flip or stretch on bad meshes; PCA boundary preservation may self-overlap or distort necks. | `omega_boundary_mode`, `omega_boundary_forced_rectangle`, `omega_boundary_constraint_model`, `boundary_self_intersection_count` | Add robust flip prevention and compare against BFF. |
