...
| S -> Omega parameterization | Paper uses a conformal/BFF-style surface parameterization. | Default is `bff`: a local boundary-first solve that lays out the boundary from 3D boundary edge lengths and signed turning angles without closure-drift correction, then solves the interior with cotangent harmonic extension. `lscm_paper_like` is retained only as an explicit comparison mode. `pca_debug`, `shape_preserving_experimental`, and `rectangular_debug` require `allow_experimental_pipeline=True`. | `_build_surface_parameterization`, `_bff_boundary_first_uv`, `_bff_boundary_polygon` in `onestring_pipeline.py` | Local BFF-style implementation | This is no longer PCA/height-field/rectangle forcing, but it is not a libigl reference BFF binding; numerical equivalence to the paper must be checked with `bff_*` metrics. | `parameterization_method`, `parameterization_exactness_label`, `omega_boundary_constraint_model`, `bff_variant`, `bff_boundary_*` | Compare against a reference BFF implementation if exact reproducibility is required. |
| Omega boundary | Boundary should come from the conformal solve, not arbitrary square forcing. | Default BFF constructs the Omega boundary first from the target surface boundary. It does not force a square, circle, projected PCA outline, or closure-drift redistribution. `shape_preserving_experimental` preserves projected outline only in explicit experimental mode; `rectangular_debug` is explicitly debug. | `_bff_boundary_polygon`, `_shape_preserving_projected_uv` | Local BFF-style / Experimental / Debug | Without closure correction, inconsistent boundary turns can leave visible closure drift; cotangent weights can still distort bad or concave meshes. Metrics expose both cases. | `omega_boundary_mode`, `omega_boundary_forced_rectangle`, `omega_boundary_constraint_model`, `bff_boundary_closure_correction_applied`, `bff_boundary_closure_drift`, `boundary_self_intersection_count` | Add reference BFF comparison and flip-prevention diagnostics. |
