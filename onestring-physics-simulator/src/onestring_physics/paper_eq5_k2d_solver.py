...
The input M2D remains the ordinary shared-vertex quad mesh. For this mode every
quad is duplicated into an independent tile. The shared M2D connectivity is
then used only to recover the cyclic order of incident tile corners and the
actual openings between consecutive tiles. EFab is evaluated on those openings,
not on arbitrary pairs in a hinge group and never on a tile interior angle.
"""
...
def _rot(v, angle):
    ...
    return np.asarray([c*v[0]-s*v[1],s*v[0]+c*v[1]],float)
...
def _build_auxetic_topology(mesh_2d):
    """Independent tiles + physical gap openings recovered from M2D connectivity.

    At a source vertex each incident tile owns one corner and two boundary rays.
    Sorting the incident tile corners by their corner-bisector direction gives
    the cyclic tile order. For each consecutive tile pair we choose the two
    boundary rays that face the angular sector between those tiles. This yields
    exactly one gap-angle constraint per physical opening, instead of all ray
    combinations at a hinge.
    """
    src_xy=np.asarray(mesh_2d.vertices[:,:2],float); src_faces=np.asarray(mesh_2d.faces,int)
    nf=len(src_faces); xy=np.zeros((4*nf,2),float); faces=np.arange(4*nf,dtype=int).reshape(nf,4)
    source_ids=np.zeros(4*nf,int); groups={}; corner_info={}
    for f,face in enumerate(src_faces):
        for local,source in enumerate(face):
            q=4*f+local; source=int(source); xy[q]=src_xy[source]; source_ids[q]=source
            groups.setdefault(source,[]).append(q)
            prev=4*f+(local-1)%4; nxt=4*f+(local+1)%4
            corner_info[q]=(f,local,prev,nxt)
    ...
    for source,copies in groups.items():
        ...
        for q in copies:
            f,local,prev,nxt=corner_info[q]
            vp=xy[prev]-center; vn=xy[nxt]-center
            ...
            phi=math.atan2(float(bis[1]),float(bis[0]))
            rays=[(math.atan2(float(vp[1]),float(vp[0])),prev),(math.atan2(float(vn[1]),float(vn[0])),nxt)]
            entries.append((phi,q,rays))
        entries.sort(key=lambda e:e[0]); m=len(entries)
        for i in range(m):
            phi_a,qa,rays_a=entries[i]; phi_b,qb,rays_b=entries[(i+1)%m]
            sector=(phi_b-phi_a)%(2*math.pi)
            if sector<=1e-9 or sector>=math.pi+1e-6: continue
            def rel(angle): return (angle-phi_a)%(2*math.pi)
            cand_a=[(rel(a),n) for a,n in rays_a if rel(a)<=sector+1e-7]
            cand_b=[(rel(a),n) for a,n in rays_b if rel(a)<=sector+1e-7]
            if not cand_a or not cand_b: continue
            na=max(cand_a,key=lambda x:x[0])[1]
            nb=min(cand_b,key=lambda x:x[0])[1]
            if na==nb: continue
            gap_pairs.append((qa,na,qb,nb))
    hinge_groups=[np.asarray(v,int) for v in groups.values() if len(v)>1]
...
def _project_edges(xy,faces,source_ids,mesh_3d):
    out=xy.copy(); acc=np.zeros_like(out); counts=np.zeros((len(out),1)); err=0.0
    for face in faces:
        for k in range(4):
            a,b=int(face[k]),int(face[(k+1)%4]); sa,sb=int(source_ids[a]),int(source_ids[b])
            target=float(np.linalg.norm(mesh_3d.vertices[sa]-mesh_3d.vertices[sb])); d=out[b]-out[a]; length=float(np.linalg.norm(d))
...
def _project_fab(xy,gap_pairs,theta_min):
    ...
    for ca,na,cb,nb in gap_pairs:
        ...
        violations+=1; cross=float(va[0]*vb[1]-va[1]*vb[0]); orient=1. if cross>=0 else -1.; total=la2+lb2
        qa=out[ca]+_rot(va,-orient*delta*(lb2/total)); qb=out[cb]+_rot(vb,orient*delta*(la2/total))
        accum[na]+=qa-out[na]; accum[nb]+=qb-out[nb]; counts[na]+=1; counts[nb]+=1; energy+=float(np.sum((qa-out[na])**2)+np.sum((qb-out[nb])**2))
    active=counts[:,0]>0; out[active]+=accum[active]/counts[active]
    return out,energy,violations,(min(angles) if angles else 0.),(max(angles) if angles else 0.)
...
def optimize_paper_eq5(mesh_2d,mesh_3d,params,*,progress_callback=None,pipeline=None):
    ...
    print(f'[PAPER-EQ5-K2D] auxetic=True physical_gaps={len(gap_pairs)} collisions={final_collisions} fab_violations={final_fab_violations} gap_min_deg={math.degrees(amin):.4g} gap_max_deg={math.degrees(amax):.4g} edge_mean={edge_mean:.6g}')
    return out,report
