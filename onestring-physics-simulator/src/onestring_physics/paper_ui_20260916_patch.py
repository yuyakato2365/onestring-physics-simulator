...
"Pipeline Optimization":("Optimization objectives","E<sub>Assembled</sub>(v)=ω₁E<sub>Planar</sub>+ω₂E<sub>Square</sub>+ω₃E<sub>Surface</sub><br><br>E<sub>Flat</sub>(v)=ω₁E<sub>Edge</sub>+ω₂E<sub>Collision</sub>+ω₃E<sub>Fab</sub>","上: M₃D→K₃D (Sec.4.2) / 下: M₂D→K₂D (Sec.4.3)。iterations/time budget は solver 設定。"),
...
"High-Fidelity Physical Mode":("Physical extensions","E=E<sub>base</sub>+E<sub>gravity</sub>+E<sub>friction</sub>+E<sub>hinge</sub>","Paper objective そのものではない追加近似。","Implementation")}
CONTROL_CARDS={
...
"3D optimization iterations":("K₃D solver","min E<sub>Assembled</sub>(v)","反復上限。","Solver"),
...
def _fig(data,a,q):
 ...
 if a in BOX:
  x,y,w,h=BOX[a];deg=max(0,min(1,float(q or 0)))*360;ov=f"<div class='os-hi' style='left:{x}%;top:{y}%;width:{w}%;height:{h}%;--p:{deg}deg'></div>"
...
def install_paper_ui_20260916_patch():
 ...
 def wrap(base):
  def f(label,*a,**k):
   key=str(label)
   if key in CONTROL_CARDS:md(_card(*CONTROL_CARDS[key]),unsafe_allow_html=True)
