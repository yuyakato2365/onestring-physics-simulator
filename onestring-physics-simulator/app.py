...
with st.sidebar:
    ...
    target_kind = _param_row(
        ...
        lambda: st.selectbox("target shape", ["dome", "flat", "half_gourd", "saddle", "wave", "gaussian"], help="Built-in target surface S. wave is damped; half_gourd is a non-rectangular cut hyotan/gourd shell."),
    )
    uploaded = _param_row(
    ...
    st.markdown(
        '<div class="param-section-note">Strict paper-flow mode: GPU-first / analytic height-field shortcuts are disabled. Ω and M3D use the stored S↔Ω parameterization.</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="param-section-note">M3D construction: mesh_harmonic only. The analytic height-field debug path is disabled in strict paper-flow mode.</div>',
        unsafe_allow_html=True,
    )
    w_planar = _param_row(
...
if "onestring_state" not in st.session_state and not run_pipeline:
    st.info("Set parameters, then click Run OneString pipeline. Strict paper-flow order is enforced; analytic/GPU-first shortcuts are disabled.")
    st.stop()
...
if view_stage == "Pipeline View":
    ...
    if not omega_info["omega_corresponds_to_S"]:
        st.error("Ω is not a paper-like conformal parameterization in this run. Disable GPU-first mode / analytic debug to inspect c:S→Ω.")
    ...
    st.write({
        "current_implementation_policy": [
            "Strict paper-flow disables analytic/GPU-first shortcuts; S is parameterized into Ω with the stored mesh UV map.",
