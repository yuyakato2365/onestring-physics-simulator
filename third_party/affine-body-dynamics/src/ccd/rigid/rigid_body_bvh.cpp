    ...
    const double inflation_radius)
{
    ...
    for (size_t fa_id = 0; fa_id < FA.rows(); fa_id++) {
        ...
        bodyB.bvh.intersect_box(
            ...
            ids);
        // std::cout << "after intersect box\n";
        for (const auto& id : ids) {
            // std::cout << "id of B is : " << id << " \n";
            if (id < bodyB.num_codim_vertices()) {
                // std::cout << "id of B is : " << id << " . It is vertex \n";
                ...
                // std::cout << "id of B is : " << id << " . It is edge \n";
                ...
                // std::cout << "id of B is : " << id << " . It is face \n";
                size_t fb_id =
                    id - bodyB.num_codim_vertices() - bodyB.num_codim_edges();
...
void detect_body_pair_collision_candidates_intersection_region(
    const RigidBodyAssembler& bodies,
    const int bodyA_id,
    const int bodyB_id,
    const int collision_types,
    Candidates& candidates,
    const double inflation_radius, 
    const std::set<int>& iVA, const std::set<int>& iEA, const std::set<int>& iFA, 
    const std::set<int>& iVB, const std::set<int>& iEB, const std::set<int>& iFB,
    std::vector<AABB>& bodyA_vertex_aabbs)
{
    bool build_ev = collision_types & CollisionType::EDGE_VERTEX;
    bool build_ee = collision_types & CollisionType::EDGE_EDGE;
    bool build_fv = collision_types & CollisionType::FACE_VERTEX;
    ...
    // const std::vector<AABB> bodyA_vertex_aabbs =
    //     vertex_aabbs(bodyA.vertices, inflation_radius);
    // const std::vector<AABB> bodyB_vertex_aabbs =
    //     vertex_aabbs(bodyB.vertices, inflation_radius);
    const Eigen::MatrixXi &EA = bodyA.edges, &EB = bodyB.edges,
                          &FA = bodyA.faces, &FB = bodyB.faces;
    ...
    // std::map<int, AABB> bodyA_vertex_aabbs;
    // for (int va_id : iVA){
    //     VectorMax3d v = bodyA.vertices.row(va_id);
    //     bodyA_vertex_aabbs[va_id] = AABB(v.array() - inflation_radius, v.array() + inflation_radius); 
    // }
    std::map<int, AABB> bodyB_vertex_aabbs;
    for (int vb_id=0; vb_id<bodyB.num_vertices(); vb_id++){
        VectorMax3d v = bodyB.vertices.row(vb_id);
        bodyB_vertex_aabbs[vb_id] = AABB(v.array() - inflation_radius, v.array() + inflation_radius); 
    }
    ///////////////////////////////////////////////////////////////////////////
    // query (f, *)
    for (int fa_id : iFA) { //(size_t fa_id = 0; fa_id < FA.rows(); fa_id++) {
        // all (f_e, *v) and (f_v, *e) are not needed because faces are only 3D
        assert(!build_ev);
        // Construct a bbox of bodyA's face
        AABB fa_aabb(bodyA_vertex_aabbs[FA(fa_id, 0)], bodyA_vertex_aabbs[FA(fa_id, 1)],
            bodyA_vertex_aabbs[FA(fa_id, 2)]);
        for (int vb_id : iVB)
        {
            if (fa_aabb.intersects(bodyB_vertex_aabbs[vb_id]))
                add_fv(fa_id, vb_id);
    ...
    for (int ea_id : iEA) { //(size_t fa_id = 0; fa_id < FA.rows(); fa_id++) {
        // all (f_e, *v) and (f_v, *e) are not needed because faces are only 3D
        assert(!build_ev);
        // Construct a bbox of bodyA's face
        AABB ea_aabb = AABB(bodyA_vertex_aabbs[EA(ea_id, 0)], bodyA_vertex_aabbs[EA(ea_id, 1)]);
        for (int eb_id : iEB)
        {
            AABB eb_aabb(bodyB_vertex_aabbs[EB(eb_id, 0)], bodyB_vertex_aabbs[EB(eb_id, 1)]);
            if (ea_aabb.intersects(eb_aabb))
                add_ee(ea_id, eb_id);
        }
    }
    for (int va_id : iVA) { //(size_t fa_id = 0; fa_id < FA.rows(); fa_id++) {
        // all (f_e, *v) and (f_v, *e) are not needed because faces are only 3D
        assert(!build_ev);
        // Construct a bbox of bodyA's face
        AABB va_aabb = bodyA_vertex_aabbs[va_id];
        for (int fb_id : iFB)
        {
            AABB fb_aabb(bodyB_vertex_aabbs[FB(fb_id, 0)], bodyB_vertex_aabbs[FB(fb_id, 1)],
            bodyB_vertex_aabbs[FB(fb_id, 2)]);
            if (fb_aabb.intersects(va_aabb))
                add_vf(va_id, fb_id);
        }
    }
}

void detect_body_pair_collision_candidates_intersection_bvh(
    const RigidBodyAssembler& bodies,
    const int bodyA_id,
    const int bodyB_id,
    const int collision_types,
        ...
        for (const auto& id : ids) {
            if (id < bodyB.num_codim_vertices()) {
                ...
                for (int ei = 0; ei < FA.cols(); ei++) {
                    size_t ea_id = selectorA.face_to_edge(fa_id, ei);
                    if (selectorA.edge_to_face(ea_id) == fa_id) {
                        AABB ea_aabb(bodyA_vertex_aabbs[EB(ea_id, 0)], bodyA_vertex_aabbs[EB(ea_id, 1)]);
                        if (ea_aabb.intersects(ea_aabb)) {
                            add_ee(ea_id, eb_id);
                        }
                    }
                ...
                AABB fb_aabb(bodyB_vertex_aabbs[FB(fb_id, 0)], bodyB_vertex_aabbs[FB(fb_id, 1)],
            bodyB_vertex_aabbs[FB(fb_id, 2)]); //= bodyB_face_aabb(fb_id);
                for (int f_vi = 0; f_vi < FA.cols(); f_vi++) {
                    // (f_v, f)
                    long va_id = FA(fa_id, f_vi);
                    if (selectorA.vertex_to_face(va_id) == fa_id) {
                ...
                for (int fa_ei = 0; fa_ei < FA.cols(); fa_ei++) {
                    ...
                    AABB ea_aabb(bodyA_vertex_aabbs[EA(ea_id, 0)], bodyA_vertex_aabbs[EA(ea_id, 0)]);// = bodyA_edge_aabb(ea_id);
                    ...
                    for (int fb_ei = 0; fb_ei < FB.cols(); fb_ei++) {
                        ...
                        AABB eb_aabb(bodyB_vertex_aabbs[EB(eb_id, 0)], bodyB_vertex_aabbs[EB(eb_id, 0)]);// = bodyB_edge_aabb(eb_id);
