...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < bodies.num_bodies(); i++) {
        ...
        from_json(jrb["rotation"], poses_t0[i].transform);
    }
    ...
    for (size_t i = 1; i < state_sequence.size(); ++i) {
        ...
        for (int j = 0; j < bodies.num_bodies(); j++) {
            ...
            from_json(jrb["rotation"], poses_t1[j].transform);
        }

        Impacts impacts;
        detect_collisions(
            bodies, poses_t0, poses_t1, collision_types, impacts,
            DetectionMethod::IAABB, TrajectoryType::ACCD); // rigidIPC default is below
