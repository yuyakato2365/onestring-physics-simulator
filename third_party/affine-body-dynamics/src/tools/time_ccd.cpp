...
int main(int argc, char* argv[])
{
    ...
    fv_csv << "rigid,pl,accd\n";
    std::ofstream ee_csv("ee.csv");
    ee_csv << "rigid,pl\n";

    std::array<TrajectoryType, 2> traj_types = { {RIGID, ACCD } };
    static double RIGID_TOL = 1e-4; // REDON_TOL = 1e-4, 
    ...
    for (int i = 0; i < 60; i++) {
        ...
        for (const auto& query : queries) {
            ...
            from_json(poseA_t0_json["rotation"], bodyA_pose_t0.transform);
            from_json(poseA_t1_json["position"], bodyA_pose_t1.position);
            from_json(poseA_t1_json["rotation"], bodyA_pose_t1.transform);
            from_json(poseB_t0_json["position"], bodyB_pose_t0.position);
            from_json(poseB_t0_json["rotation"], bodyB_pose_t0.transform);
            from_json(poseB_t1_json["position"], bodyB_pose_t1.position);
            from_json(poseB_t1_json["rotation"], bodyB_pose_t1.transform);
            ...
            for (int i = 0; i < traj_types.size(); i++) {
                ...
                if (ccd_type == "ee") {
                    ...
                    //     break;
                    case ACCD:
                        Eigen::Vector3d ea0 = bodyA.world_vertex(
                            bodyA_pose_t0, bodyA_edges(0, 0));
                        Eigen::Vector3d dea0 =
                            bodyA.world_vertex(bodyA_pose_t1, bodyA_edges(0, 0))
                            - ea0;

                        Eigen::Vector3d ea1 = bodyA.world_vertex(
                            bodyA_pose_t0, bodyA_edges(0, 1));
                        Eigen::Vector3d dea1 =
                            bodyA.world_vertex(bodyA_pose_t1, bodyA_edges(0, 1))
                            - ea1;

                        Eigen::Vector3d eb0 = bodyB.world_vertex(
                            bodyB_pose_t0, bodyB_edges(0, 0));
                        Eigen::Vector3d deb0 =
                            bodyB.world_vertex(bodyB_pose_t1, bodyB_edges(0, 0))
                            - eb0;

                        Eigen::Vector3d eb1 = bodyB.world_vertex(
                            bodyB_pose_t0, bodyB_edges(0, 1));
                        Eigen::Vector3d deb1 =
                            bodyB.world_vertex(bodyB_pose_t1, bodyB_edges(0, 1))
                            - eb1;
                        // @javidf: what is conservative rescaling? accuracy
                        // measure? PLEASE CHECK THIS AGAIN.
                        toi = 1.0;
                        bool result = accd::Edge_Edge_CCD(
                            ea0, ea1, eb0, eb1, dea0, dea1, deb0, deb1,
                            /*conservative_rescaling=*/0.001, /*thickness=*/0.0,
                            toi);
                        break;
                    }
                } else if (ccd_type == "fv") {
