...
void convert_separators_to_native(std::string& path){
#ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
    // If filesystem is windows, convert "/" separators to "\\"
    std::replace(path.begin(), path.end(), L'/', L'\\');
#else
    // If not, convert windows separators "\\" to unix ones "/"
    // Note that "\\" could be a legitimate part of a filename in unix although it's rare
    std::replace(path.begin(), path.end(), L'\\', L'/');
#endif
}
...
bool read_rb_scene(const nlohmann::json& scene, std::vector<RigidBody>& rbs)
{
    ...
    for (auto& jrb : scene["rigid_bodies"]) {
        ...
        json args = R"({
            ...
            })"_json; // "is_dof_fixed": [false, false, false, false, false, false], this is cut from args after "density".
        ...
        if (mesh_fname != "") {
            convert_separators_to_native(mesh_fname);
            fs::path mesh_path(mesh_fname);

            if (!exists(mesh_path)) {
                // TODO: First check a path relative to the input file
                mesh_path = fs::path(ABD_MESHES_DIR) / mesh_path;
            }
        ...
        // TODO: Currently all constraints are handled by the constraint handler
        ...
        for (const auto& json_pose : json_kinematic_poses) {
            ...
            if (json_pose.contains("rotation")) {
                VectorMax3d rotation(pose.dim());
                from_json(json_pose["rotation"], rotation);
        ...
        if (args["split_components"].get<bool>()) {
            ...
            for (int ci = 0; ci < CFs.size(); ci++) {
                ...
                rbs.emplace_back(
                    CV, CE, CF, PoseD::FromRigidPose(position, VectorMax3d::Zero(angular_dim)),
                    PoseD::FromRigidVelocity(linear_velocity, angular_velocity, VectorMax3d::Zero(angular_dim)),
                    PoseD::Zero(dim), density, is_oriented,
                    group_id, rb_type, kinematic_max_time, kinematic_poses);
                rbs.back().name = fmt::format("{}-part{:03d}", rb_name, ci);
                rbs.back().add_force(VectorMax3d::Zero(dim), force);
                rbs.back().add_torque(torque);
            }
        } else {
            // @javidf why is pose redefined here with position and zero Vector here?
            rbs.emplace_back(
                vertices, edges, faces,
                PoseD::FromRigidPose(position, VectorMax3d::Zero(angular_dim)),
                PoseD::FromRigidVelocity(linear_velocity, angular_velocity, VectorMax3d::Zero(angular_dim)), PoseD::Zero(dim),
                density, is_oriented, group_id, rb_type,
