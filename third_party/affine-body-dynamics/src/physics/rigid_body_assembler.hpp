...
public:
    ...
    MatrixX<T> world_vertices(
        const std::vector<MatrixMax3<T>>& transforms,
    ...
    /// @brief Initializes the jacobian of the vertices with repect to affine body DOF (constant).
    ///
    /// stored in a condensed form where only the
    /// affine dof of the body the vertex belongs to is computed.
    void init_world_vertices_jacobian();

    /// @brief Derivatives of the vertices with repect to affine body DOF.
    ///
        ...
        bool compute_jac) const
    {
        return world_vertices_diff(
            PoseD::dofs_to_poses(dof, dim()), jac, compute_jac);
    }
    ...
    long body_name_to_id(std::string name) const
    {
        auto it = body_name_id_map.find(name);
        return it != body_name_id_map.end() ? it->second : -1;
    }
    ...
    std::vector<std::pair<int, int>> close_bodies_bvh(
        ...
        const PosesD& poses_t1,
        const double inflation_radius) const;
    std::vector<std::pair<int, int>> close_bodies_iaabb(
        const PosesD& poses_t0,
        const PosesD& poses_t1,
        const double inflation_radius) const;
    // @javidf: This might be removed, if possible?
    std::vector<std::pair<int, int>> close_bodies_iaabb(
        const PosesD& poses,
        const double inflation_radius) const;
    ...
    /// @brief body name to id map
    std::map<std::string, long> body_name_id_map;
    ...
    // @javidf: this is added to be able to run the code while movging from rigid to ABD
    VectorXb is_rb_dof_fixed;// = VectorXb::Zero(num_bodies() * );

protected:
    /// @brief Group ids per vertex
    Eigen::VectorXi m_vertex_group_ids;

    /// @brief Jacobian of world vertices (all bodies) wrt poses
    // Note that this is constant, thus created at initialization and stored
    Eigen::MatrixXd m_world_vertices_jac;
};
