...
assembly = NS(
    vertices=np.asarray([tile, tile + [1.2, 0.0, 0.0]]),
    metrics={"tile_thickness": 0.1},
)
hinge = NS(
    tile_a=0,
    tile_b=1,
...
gaps = [
    NS(
        id=0,
        surrounding_tiles=[0],
        centroid_2d=np.asarray([0.0, 0.0]),
        centroid_3d=np.asarray([0.0, 0.0, 0.0]),
    ),
    NS(
        id=1,
        surrounding_tiles=[1],
        centroid_2d=np.asarray([1.2, 0.0]),
        centroid_3d=np.asarray([1.2, 0.0, 0.0]),
    ),
]
state = NS(
    tiles_3d=assembly,
...
job = prepare_abd_job(
    state,
    ABDBackendConfig(steps=12),
    ROOT / "output" / "abd_bridge_smoke",
)
...
assert manifest["schema"] == "onestring-abd-bridge-v2"
assert manifest["string"]["inequality"] == "L(q) <= L_command(t)"
assert manifest["string"]["path_points"][0]["type"] == "world_anchor"
assert manifest["string"]["path_points"][0]["id"] == "support"
assert len(manifest["string"]["path_points"]) == 3
print(
    json.dumps(
        {
            "scene": str(job.scene_path),
            "manifest": str(job.manifest_path),
            "guides": job.guide_count,
            "path_points": len(manifest["string"]["path_points"]),
            "bodies": 2,
        },
        indent=2,
    )
